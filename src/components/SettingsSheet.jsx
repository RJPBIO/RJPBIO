...
function Toggle({ on, onToggle, ac, bd }) {
  return (
    <button
      ...
      style={{ width: 42, height: 24, borderRadius: 12, background: on ? ac : bd, cursor: "pointer", position: "relative", transition: "background .3s", border: "none", padding: 0, minHeight: 24 }}
    >
      <div style={{ width: 20, height: 20, borderRadius: 10, background: "#fff", position: "absolute", top: 2, left: on ? 20 : 2, transition: "left .3s", boxShadow: "0 1px 3px rgba(0,0,0,.15)" }} />
    </button>
  );
}
...
export default function SettingsSheet({
  ...
  function handleExportJSON() {
    try {
      ...
      console.error("[BIO] JSON export error:", e.message);
  ...
  function handleExportNOM035() {
    try {
      ...
      console.error("[BIO] NOM-035 export error:", e.message);
  ...
  return (
    <AnimatePresence>
    {show&&(<motion.div role="dialog" aria-modal="true" aria-label="Configuración" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} style={{position:"fixed",inset:0,zIndex:z.overlay,background:"rgba(15,23,42,.3)",backdropFilter:"blur(16px)",display:"flex",alignItems:"flex-end",justifyContent:"center"}} onClick={onClose}>
      ...
      {[{l:"Sonido + ambiente",k:"soundOn",d:"Acordes, ruido ambiental y binaural",ic:"volume-on"},{l:"Vibración",k:"hapticOn",d:"Feedback háptico neurosensorial",ic:"vibrate"},{l:"Voz guiada",k:"_voice",d:"Narración de fases y respiración",ic:"mind"}].map(s=>(
        <div key={s.k} style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:`${space[3]}px 0`,borderBottom:`1px solid ${bd}`}}><div style={{display:"flex",alignItems:"center",gap:space[2]}}><Icon name={s.ic} size={15} color={t3}/><div><div style={ty.title(t1)}>{s.l}</div><div style={{...ty.caption(t3),marginTop:1}}>{s.d}</div></div></div>
          <Toggle on={s.k==="_voice"?voiceOn:st[s.k]!==false} onToggle={()=>{if(s.k==="_voice"){setVoiceOn(!voiceOn);}else setSt({...st,[s.k]:st[s.k]===false?true:false});}} ac={ac} bd={bd} />
        </div>))}
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:`${space[3]}px 0`,borderBottom:`1px solid ${bd}`}}><div style={{display:"flex",alignItems:"center",gap:space[2]}}><Icon name="palette" size={15} color={t3}/><div style={ty.title(t1)}>Tema</div></div><div style={{display:"flex",gap:space[1]}}>{["auto","light","dark"].map(m=>(<button key={m} aria-pressed={(st.themeMode||"auto")===m} onClick={()=>setSt({...st,themeMode:m})} style={{padding:`${space[1]}px ${space[3]}px`,borderRadius:radius.sm-1,border:`1px solid ${(st.themeMode||"auto")===m?ac:bd}`,background:(st.themeMode||"auto")===m?withAlpha(ac,6):cd,color:(st.themeMode||"auto")===m?ac:t3,...ty.caption((st.themeMode||"auto")===m?ac:t3),cursor:"pointer",textTransform:"capitalize",minHeight:32}}>{m}</button>))}</div></div>
      ...
      <div style={{padding:"13px 0",borderBottom:`1px solid ${bd}`}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:6}}>
          {SOUNDSCAPES.map(s=>{const unlocked=(st.unlockedSS||["off"]).includes(s.id);const active=(st.soundscape||"off")===s.id;return<motion.button key={s.id} aria-label={`${s.n}${active?" activo":""}${!unlocked?` — ${s.cost} V-Cores`:""}`} whileTap={{scale:.95}} onClick={()=>{if(unlocked){setSt({...st,soundscape:s.id});H("tap");}else if((st.vCores||0)>=s.cost){setSt({...st,soundscape:s.id,unlockedSS:[...(st.unlockedSS||["off"]),s.id],vCores:(st.vCores||0)-s.cost});H("ok");}}} style={{padding:"10px 8px",borderRadius:12,border:active?`2px solid ${ac}`:unlocked?`1.5px solid ${bd}`:`1.5px dashed ${bd}`,background:active?withAlpha(ac,4):cd,cursor:unlocked||(st.vCores||0)>=s.cost?"pointer":"not-allowed",opacity:unlocked||(st.vCores||0)>=s.cost?1:.5,textAlign:"center"}}>
...
function exportNOM035(st) {
  ...
  // Sanitize numeric values for HTML
  ...
  const html = `<!DOCTYPE html><html lang="es"><head><meta charset="UTF-8"><title>Informe NOM-035</title><style>*{margin:0;padding:0;box-sizing:border-box}body{font-family:-apple-system,sans-serif;color:#0F172A;background:#fff;padding:40px;max-width:800px;margin:0 auto;font-size:14px}.header{border-bottom:3px solid #059669;padding-bottom:20px;margin-bottom:30px}.logo{font-size:24px;font-weight:800;color:#059669}h2{font-size:16px;margin:28px 0 14px;border-bottom:1px solid #E2E8F0;padding-bottom:6px}.grid{display:grid;grid-template-columns:1fr 1fr 1fr;gap:12px;margin-bottom:20px}.card{background:#F8FAFC;border:1px solid #E2E8F0;border-radius:10px;padding:14px}.card .v{font-size:22px;font-weight:800}.card .l{font-size:10px;color:#64748B;margin-top:2px;text-transform:uppercase}.imp{font-size:28px;font-weight:800;color:${safeDelta >= 0 ? "#059669" : "#DC2626"};text-align:center;padding:20px;background:${safeDelta >= 0 ? "#F0FDF4" : "#FEF2F2"};border-radius:12px;margin-bottom:20px}.footer{margin-top:40px;border-top:2px solid #E2E8F0;padding-top:16px;font-size:10px;color:#94A3B8;text-align:center}</style></head><body><div class="header"><div class="logo">BIO-IGNICIÓN</div><div style="font-size:11px;color:#64748B;margin-top:4px">Informe de Bienestar Laboral — NOM-035-STPS-2018</div><div style="font-size:11px;color:#475569;margin-top:8px">Fecha: ${now.toLocaleDateString("es-MX", { year: "numeric", month: "long", day: "numeric" })}</div></div><h2>Resumen</h2><div class="grid"><div class="card"><div class="v">${safeSessions}</div><div class="l">Sesiones</div></div><div class="card"><div class="v">${safeTotalMin}min</div><div class="l">Tiempo</div></div><div class="card"><div class="v">${uniqueDays}</div><div class="l">Días activos</div></div></div><div class="imp">${safeDelta >= 0 ? "+" : ""}${safeDelta} puntos<br><span style="font-size:11px;font-weight:400;color:#64748B">Mejora promedio por sesión</span></div><h2>Protocolos</h2>${topProtos.map(([n, c]) => `<div style="display:flex;justify-content:space-between;padding:8px 0;border-bottom:1px solid #F1F5F9"><span>${n}</span><span>${c}x (${Math.round(c / safeSessions * 100)}%)</span></div>`).join("")}<h2>Riesgo</h2><div style="padding:14px;background:${safeRiskPct > 30 ? "#FEF2F2" : "#F0FDF4"};border-radius:10px;margin-bottom:20px"><div style="font-size:20px;font-weight:800;color:${safeRiskPct > 30 ? "#DC2626" : "#059669"}">${safeRiskPct}%</div><div style="font-size:11px;color:${safeRiskPct > 30 ? "#DC2626" : "#059669"}">Sesiones con tensión alta</div></div><div class="footer"><p><strong>BIO-IGNICIÓN</strong> — Generado: ${now.toLocaleString("es-MX")}</p></div></body></html>`;
