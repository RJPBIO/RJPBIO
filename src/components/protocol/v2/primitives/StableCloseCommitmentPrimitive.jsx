"use client";
/* ═══════════════════════════════════════════════════════════════
   StableCloseCommitmentPrimitive — Phase 7 SP-G-3
   ───────────────────────────────────────────────────────────────
   Primitive dedicated para Phase 3 "Cierre Estable" del protocolo
   #6 Grounded Steel. Reemplaza shared hold_press_button con
   primitive multi-exercise wrapper STABLE-PRESENCE-driven.

   Diferenciación vs Tier 1A+1B Phase 3 commitment primitives:
     - #1 CommitmentMotor: parasympathetic + "Cierra puño libre"
     - #2 VisualizationCommitment: cognitive + bilateral saccades
     - #3 ExecutiveCommitment: triple-seal + 60-min ultradian
     - #4 EnergyAnchorCommitment: energy-driven + postura activa
     - #5 FocusCommitment: visual-anchor + mirada firme
     - #6 StableCloseCommitment: STABLE-PRESENCE-driven + palma muslo

   Body anchor "palma libre firme contra el muslo" del catálogo
   #6 — FREE-HAND-FRIENDLY ✅ (palma presiona SUPERFICIE EXTERNA
   muslo, no juntas) — compatible con hold-press del pulgar.

   Macro-phase A→B (5s + 25s):
     Phase A — Postura + Palma Setup (0-5s):
       - Primary prompt: "Palma libre firme contra el muslo · Postura erguida"
       - Body anchor: "Postura preparada"
       - Hold-press hidden.
     Phase B — Hold + Verbalización (5-30s):
       - Primary prompt: "Estoy aquí. Sigo firme."
       - Hold-press 6s con ring progress.
       - Body anchor: "Palma libre firme en muslo" sustained.

   Multi-exercise tracks layered (6):
     1. POSTURA física sustained: "Palma libre firme en muslo" anchor
        proprioceptivo (mirror catálogo cite "palmas firmes contra los
        muslos" adaptado para 1-mano celular).
     2. MOTOR primary: hold-press 6s con ring progress (anti-trampa).
     3. MENTAL anchor: "Estoy aquí. Sigo firme." verbalización mental
        prominent — mantra de presencia ejecutiva.
     4. VISUAL continuity: orb continuation Phase 1+2 carry-over.
     5. VISUAL particles: centrifugal projection.
     6. PHASE label "Cierre Estable" cyan-warm.

   Mecanismos científicos (NO surface UI per user feedback):
     - Anclaje motor + verbalización mental consolida estado calmado
       (Bryan, Adams, Monin 2013 implementation intentions).
     - Postura propioceptiva sostenida = grounding ejecutivo
       (Carney Cuddy Yap 2010 cognitive priming).

   Functional human logic:
     - Phase A: postura + palma libre firme (somatic prep 5s).
     - Phase B: hold-press pulgar (motor 1) + palma muslo (motor 2 +
       somatic) + mantra mental "Estoy aquí. Sigo firme." (cognitive)
       — tri-modal seal stable.

   Constraint compliance:
     - Oficina + sentado + sin volumen + 1 mano celular: ✅ todos.
     - Hold-press pulgar mano celular; otra mano libre presiona muslo
       (no conflict palmas juntas).
   ═══════════════════════════════════════════════════════════════ */
...
const PHASE_LABEL = "Cierre Estable";
const PHASE_A_PROMPT = "Palma libre firme contra el muslo · Postura erguida";
const PHASE_B_PROMPT = "Estoy aquí. Sigo firme.";
const PHASE_A_BODY_ANCHOR = "Postura preparada";
const PHASE_B_BODY_ANCHOR = "Palma libre firme en muslo";
 ...
 * @param {string} [props.release_message="Aquí. Firme."]
 ...
 */
export default function StableCloseCommitmentPrimitive({
  label = "MANTÉN",
  min_hold_ms = 6000,
  ...
  // Track 4: orb continuation (Phase 1+2 carry-over).
  ...
  // Track 5: particles centrifugal projection.
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (particleSysRef.current) {
        particleSysRef.current.setPhase("hold", 0); // grounded hold pattern (no projection — calm)
  ...
  return (
    <div
      data-v2-stable-close-commitment
      data-macro-phase={macroPhase}
      data-completed={completed ? "true" : "false"}
      data-pressing={pressing ? "true" : "false"}
      data-testid="stable-close-commitment-primitive"
      role="region"
      aria-label="Cierre Estable, palma libre firme + estoy aquí sigo firme"
      style={{
        width: "100%",
        position: "relative",
        display: "flex",
    ...
    >
      ...
      <span
        data-testid="stable-close-commitment-phase-label"
        style={{
      ...
      <p
        data-testid="stable-close-commitment-primary-prompt"
        aria-live="polite"
        style={{
          margin: 0,
          fontFamily: typography.family,
      ...
      >
        {/* Particles grounded hold pattern */}
        <canvas
          ref={particleCanvasRef}
          data-testid="stable-close-commitment-particles"
          aria-hidden="true"
          style={{
            position: "absolute",
            inset: 0,
        ...
        <div
          ref={orbRef}
          data-testid="stable-close-commitment-orb"
          aria-hidden="true"
          style={{
            position: "absolute",
            width: 200,
        ...
        {macroPhase === "B" && (
          <button
            type="button"
            data-testid="stable-close-commitment-hold-button"
            aria-label={label}
      ...
      {/* Body anchor sustained — palma libre firme en muslo (FREE-HAND) */}
      <span
        data-testid="stable-close-commitment-body-anchor"
        aria-live="polite"
        style={{
          fontFamily: typography.family,
