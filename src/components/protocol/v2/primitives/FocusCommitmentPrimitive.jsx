"use client";
/* ═══════════════════════════════════════════════════════════════
   FocusCommitmentPrimitive — Phase 7 SP-F-3
   ───────────────────────────────────────────────────────────────
   Primitive dedicated para Phase 3 "Compromiso de Enfoque" del
   protocolo #5 Skyline Focus. Reemplaza shared hold_press_button
   con primitive multi-exercise wrapper visual-anchor-driven.
   ...
   Diferenciación vs Tier 1A+1B Phase 3 commitment primitives:
     ...
     - #5 FocusCommitment: VISUAL-ANCHOR-driven + 60-min focus block.

   Body anchor único #5 = MIRADA FIRME (visual focus). NO manos extras
   (lección persistente palmas-vs-hold-press conflict). El anchor
   "palmas presionadas" del catálogo se OMITE del UI por conflicto.

   Macro-phase A→B (5s + 25s):
     Phase A — Postura + Mirada Setup (0-5s):
       - Primary prompt: "Espalda recta · Mirada firme al frente"
       - Body anchor: "Postura preparada"
       - Hold-press hidden.
     Phase B — Hora de Foco + Hold (5-30s):
   ...
   Multi-exercise tracks layered (6):
     1. VISUAL ANCHOR físico: "Mirada firme al frente" sustained body
        anchor — visual focus seal (continuidad con dual focus + paradox
        Phase 1+2).
     2. POSTURA física: "Espalda recta" Phase A setup.
     3. MOTOR primary: hold-press 5s con ring progress (anti-trampa).
     4. MENTAL anchor: "Esta es mi próxima hora de foco" prompt
        prominente — verbalización mental focus block ultradian.
     5. VISUAL continuity: orb continuation Phase 1+2 + particles
        centrifugal projection.
     6. PHASE label "Compromiso de Enfoque" cyan-warm.

   Mecanismos científicos (NO surface UI per user feedback):
     - Visual anchor + commitment motor refuerza intención de single-task
       (sustained focused gaze pre-frontal cortex activation).
     - Implementation intentions duplica probabilidad seguimiento (Bryan
       Adams Monin 2013).
     - 60-min focus block alineado con ultradian rhythm (Kleitman BRAC).

   Functional human logic:
     - Phase A: postura + mirada setup (5s prep).
     - Phase B: hold-press pulgar (motor) + mirada firme al frente
       (visual anchor sostenido) + verbalización mental.
     - SIN body anchor manos = NO conflict con hold-press.

   Constraint compliance:
     - Oficina + sentado + sin volumen + 1 mano celular: ✅ todos.
     - Hold-press pulgar; otra mano libre o relax (no conflict).
     - Mirada firme = visual anchor passive sostenido.
   ═══════════════════════════════════════════════════════════════ */
...
const PHASE_LABEL = "Compromiso de Enfoque";
const PHASE_A_PROMPT = "Espalda recta · Mirada firme al frente";
const PHASE_B_PROMPT = "Esta es mi próxima hora de foco";
const PHASE_A_BODY_ANCHOR = "Postura preparada";
const PHASE_B_BODY_ANCHOR = "Mirada firme al frente";
 ...
 * @param {string} [props.release_message="Una hora de foco."]
 ...
 */
export default function FocusCommitmentPrimitive({
  label = "MANTÉN",
  min_hold_ms = 5000,
  release_message = "Una hora de foco.",
  ...
  return (
    <div
      data-v2-focus-commitment
      data-macro-phase={macroPhase}
      data-completed={completed ? "true" : "false"}
      data-pressing={pressing ? "true" : "false"}
      data-testid="focus-commitment-primitive"
      role="region"
      aria-label="Compromiso de Enfoque, mirada firme + hora de foco"
      style={{
        width: "100%",
        position: "relative",
        display: "flex",
    ...
    >
      ...
      <span
        data-testid="focus-commitment-phase-label"
        style={{
      ...
      <p
        data-testid="focus-commitment-primary-prompt"
        aria-live="polite"
        style={{
          margin: 0,
          fontFamily: typography.family,
      ...
      >
        ...
        <canvas
          ...
          data-testid="focus-commitment-particles"
          aria-hidden="true"
          style={{
            position: "absolute",
            inset: 0,
        ...
        <div
          ref={orbRef}
          data-testid="focus-commitment-orb"
          aria-hidden="true"
          style={{
            position: "absolute",
            width: 200,
        ...
        {macroPhase === "B" && (
          <button
            type="button"
            data-testid="focus-commitment-hold-button"
            aria-label={label}
      ...
      {/* Body anchor sustained — VISUAL ANCHOR (no manos, evita conflict) */}
      <span
        data-testid="focus-commitment-body-anchor"
        aria-live="polite"
        style={{
          fontFamily: typography.family,
