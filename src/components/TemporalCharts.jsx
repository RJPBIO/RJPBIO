    ...
    <div style={{ width: "100%", height: 100 }}>
      <ResponsiveContainer width="100%" height="100%">
        <AreaChart data={data} margin={{ top: 5, right: 5, left: -20, bottom: 0 }}>
          ...
          <XAxis dataKey="date" tick={{ fontSize: font.size.xs - 1, fill: t3 }} axisLine={false} tickLine={false} interval="preserveStartEnd" />
          <YAxis domain={[0, 5]} tick={{ fontSize: font.size.xs - 1, fill: t3 }} axisLine={false} tickLine={false} ticks={[1, 3, 5]} />
          <Tooltip
            contentStyle={{
              background: cd,
              border: `1px solid ${bd}`,
              borderRadius: radius.sm + 2, fontSize: font.size.sm, padding: `${space[1.5]}px ${space[2.5]}px`,
            }}
...
export function EnergyFlowChart({ history, isDark, ac = "#059669" }) {
  ...
  return (
    <div>
      <div style={{ width: "100%", height: 70 }}>
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={data} margin={{ top: 5, right: 0, left: 0, bottom: 0 }}>
            <XAxis dataKey="hour" tick={{ fontSize: font.size.xs - 1, fill: t3 }} axisLine={false} tickLine={false} interval={2} />
            <Tooltip
              contentStyle={{ background: cd, border: `1px solid ${bd}`, borderRadius: radius.sm, fontSize: font.size.sm }}
      ...
      <div style={{ fontSize: font.size.xs, color: t2, marginTop: space[1.5], fontStyle: "italic" }}>
...
export function ActivityHeatmap({ history, isDark, ac = "#059669" }) {
  const { card: cd, surface, border: bd, t1, t3 } = resolveTheme(isDark);
  ...
  return (
    <div>
        ...
        {cells.map((cell) => (
          <div
            ...
            style={{
              ...
              background: cell.count === 0 ? surface :
                cell.count === 1 ? withAlpha(ac, 30) : cell.count === 2 ? withAlpha(ac, 60) : ac,
          ...
          >
            {cell.count > 0 && (
              <span style={{ fontSize: font.size.xs, fontWeight: font.weight.black, color: cell.count >= 3 ? "#fff" : ac }}>
                {cell.count}
              </span>
            )}
          </div>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", marginTop: space[1.5] }}>
        <span style={{ fontSize: font.size.xs, color: t3 }}>4 semanas atrás</span>
        <div style={{ display: "flex", alignItems: "center", gap: 3 }}>
          <span style={{ fontSize: font.size.xs, color: t3 }}>Menos</span>
          {[0, 1, 2, 3].map((i) => (
            <div key={i} style={{ width: 10, height: 10, borderRadius: 2, background: i === 0 ? surface : i === 1 ? withAlpha(ac, 30) : i === 2 ? withAlpha(ac, 60) : ac }} />
          ))}
          <span style={{ fontSize: font.size.xs, color: t3 }}>Más</span>
        </div>
      </div>
    </div>
  );
    ...
    <div style={{ display: "flex", alignItems: "flex-end", gap: 3, height: 50 }}>
      {data.map((d, i) => (
        <div key={i} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 2 }}>
          ...
          <span style={{ fontSize: font.size.xs, color: d.isToday ? ac : t3, fontWeight: d.isToday ? font.weight.black : font.weight.semibold }}>
            {d.day}
          </span>
        </div>
      ))}
...
export function CoherenceSparkline({ history, isDark, width = 120, height = 30 }) {
  ...
  const color = lastVal >= firstVal ? semantic.success : semantic.danger;
