...
function Ic({ name, size = 16, color = "#64748B" }) {
  const s = { width: size, height: size, viewBox: "0 0 24 24", fill: "none", style: { display: "block", flexShrink: 0 } }; const p = { stroke: color, strokeWidth: "1.8", strokeLinecap: "round", strokeLinejoin: "round" };
...
function startSoundscape(type) {
  try {
    const c = gAC(); if (!c || type === "off") return; if (c.state === "suspended") c.resume(); stopSoundscape(); _ssGain = c.createGain(); _ssGain.gain.value = 0; _ssGain.connect(c.destination);
...
function genIns(st) {
  const r = []; if (st.totalSessions > 0) {
    const cG = st.coherencia - 64; if (cG > 10) r.push({ t: "up", x: `Enfoque +${cG} puntos desde el inicio.` }); const rG = st.resiliencia - 66; if (rG > 5) r.push({ t: "up", x: `Calma sistémica +${rG}%.` }); if (st.streak >= 3) r.push({ t: "fire", x: `${st.streak} días consecutivos. El hábito se consolida.` }); if (st.totalTime > 0) r.push({ t: "star", x: `${Math.round(st.totalTime / 60)} minutos invertidos en rendimiento.` }); const ml = st.moodLog || []; if (ml.length >= 3) {
      const a = ml.slice(-3).reduce((a, m) => a + m.mood, 0) / 3; if (a >= 4) r.push({ t: "up", x: "Tendencia emocional ascendente." }); if (a <= 2) r.push({ t: "alert", x: "Tensión elevada detectada. Prioriza sesiones de Calma." });
    ...
    } if (st.history?.length >= 3) { const l3 = st.history.slice(-3); if (l3.every(h => h.p === l3[0].p)) { const o = P.find(p => p.n !== l3[0].p); if (o) r.push({ t: "rec", x: `Diversifica: prueba ${o.n}.` }); } }
  } const h = new Date().getHours(); if (!r.find(x => x.t === "rec")) r.push({ t: "rec", x: h < 12 ? "Reset Ejecutivo ideal para la mañana." : h < 17 ? "Protocolo OMEGA para la tarde." : "Reinicio Parasimpático para cerrar el día." }); if (!r.length) r.push({ t: "star", x: "Tu primera ignición te espera." }); return r;
}
...
function startBinaural(type) {
  try {
    const c = gAC(); if (!c) return; stopBinaural(); _binauralGain = c.createGain(); _binauralGain.gain.value = 0; _binauralGain.connect(c.destination);
...
function calcProtoSensitivity(ml) {
  ...
  const r = {}; Object.entries(bp).forEach(([n, d]) => {
    const a = d.d.reduce((a, b) => a + b, 0) / d.d.length;
    r[n] = { avgDelta: +a.toFixed(2), sessions: d.c, eff: a > 0.5 ? "alta" : a > 0 ? "media" : "baja" };
  }); return r;
}
...
export default function BioIgnicion() {
  ...
  useEffect(() => {
    if (typeof window === "undefined") return; try {
      const params = new URLSearchParams(window.location.search); const c = params.get("c"), t = params.get("t"), e = params.get("e"); if (c || t) {
        setNfcCtx({ company: c || "", type: t || "entrada", employee: e || "" }); setEntryDone(true);
  ...
  useEffect(() => {
    if (ts !== "running" || typeof window === "undefined" || !window.speechSynthesis) return;
  ...
  useEffect(() => {
    const totalDur = Math.round(pr.d * durMult); const el = totalDur - sec; const scale = durMult; let idx = 0; for (let i = pr.ph.length - 1; i >= 0; i--) { if (el >= Math.round(pr.ph[i].s * scale)) { idx = i; break; } }
    // Pre-announce next phase 2s before transition
    if (idx !== pi) {
      const nextIdx = idx; setPi(nextIdx); hapticPhase(pr.ph[nextIdx].ic); try { navigator.vibrate([30, 50, 30, 50, 80]); } catch (e) { } speakNow("Fase " + (nextIdx + 1) + " de " + pr.ph.length + ". " + pr.ph[nextIdx].k);// Full instruction via voice removed — too long for iOS TTS queue. Key phrase is enough.}
  ...
  function comp() {
    if (pauseTRef.current) clearTimeout(pauseTRef.current); if (motionRef.current) { motionRef.current.cleanup(); motionRef.current = null; }
  ...
  return (
    <><div style={{ maxWidth: 430, margin: "0 auto", minHeight: "100dvh", background: bg, position: "relative", overflow: "hidden", fontFamily: "'Manrope',-apple-system,sans-serif", transition: "background .8s" }}>
      ...
      <div style={{ opacity: tabFade, transition: "opacity .25s cubic-bezier(.4,0,.2,1),transform .25s", transform: tabFade === 1 ? "translateY(0)" : "translateY(8px)", position: "relative", zIndex: 1 }}>
              ...
              {isActive && (() => {
                ...
                if (isCP1) return (
                  <div style={{ marginTop: 12, animation: "fi .5s" }}>
                    <button
                      onTouchStart={(e) => {
                        e.currentTarget.dataset.holdStart = Date.now(); e.currentTarget.dataset.holding = "true"; e.currentTarget.style.transform = "scale(0.94)"; e.currentTarget.style.background = ac + "15"; e.currentTarget.style.borderColor = ac + "50"; hapticBreath("INHALA");
                      ...
                      onTouchEnd={(e) => {
                        const dur = Date.now() - (+e.currentTarget.dataset.holdStart || Date.now()); e.currentTarget.dataset.holding = "false"; e.currentTarget.style.transform = "scale(1)"; e.currentTarget.style.background = ac + "06"; e.currentTarget.style.borderColor = ac + "25";
                      ...
                      onMouseUp={(e) => {
                        const dur = Date.now() - (+e.currentTarget.dataset.holdStart || Date.now()); e.currentTarget.style.transform = "scale(1)"; const bar = e.currentTarget.querySelector("[data-hold-bar]"); if (bar) { bar.style.transition = "none"; bar.style.width = "0%"; }
                ...
                if (isCP2) return (
                  <div style={{ marginTop: 12, animation: "fi .5s" }}>
                    <button
                      ...
                      onTouchEnd={(e) => {
                        const rt = Date.now() - (+e.currentTarget.dataset.tapTime || Date.now()); e.currentTarget.style.transform = "scale(1)"; e.currentTarget.style.background = ac + "06";
                      ...
                      onClick={(_e) => { setSessionData(d => ({ ...d, interactions: (d.interactions || 0) + 0.7 })); H("tap"); }}
        ...
        {tab === "dashboard" && (<div style={{ padding: "14px 20px 180px" }}>
          {noData ? <div style={{ textAlign: "center", padding: "50px 20px" }}><Ic name="bolt" size={34} color={ac} /><div style={{ fontSize: 15, fontWeight: 800, color: t1, marginTop: 10, marginBottom: 5 }}>Tu dashboard te espera</div><div style={{ fontSize: 11, color: t3, marginBottom: 18 }}>Completa tu primera ignición.</div><button onClick={() => switchTab("ignicion")} style={{ padding: "11px 28px", borderRadius: 50, background: ac, border: "none", color: "#fff", fontSize: 11, fontWeight: 800, cursor: "pointer", letterSpacing: 2, textTransform: "uppercase" }}>IR A IGNICIÓN</button></div>
            : <>
              ...
              {/* Baseline Comparison */}
              {st.history && st.history.length >= 5 && (() => {
                const first5 = st.history.slice(0, 5); const last5 = st.history.slice(-5); const baseC = Math.round(first5.reduce((a, h) => a + (h.c || 50), 0) / 5); const nowC = Math.round(last5.reduce((a, h) => a + (h.c || 50), 0) / 5); const delta = nowC - baseC; return (<div style={{ background: delta > 0 ? (isDark ? "#0A1A0A" : "#F0FDF4") : (isDark ? "#1A0A0A" : "#FEF2F2"), borderRadius: 16, padding: "14px 12px", marginBottom: 14, border: "1.5px solid " + (delta > 0 ? "#05966920" : "#DC262620") }}>
                  ...
                  <div style={{ background: cd, borderRadius: 22, padding: "20px 16px", marginBottom: 14, border: `1px solid ${bd}`, position: "relative", overflow: "hidden" }}>
                    ...
                    <div style={{ position: "relative", width: "100%", maxWidth: 300, margin: "0 auto", aspectRatio: "1.2" }}>
                      <svg viewBox="0 0 300 250" style={{ width: "100%", height: "100%" }}>
                        ...
                        {/* Sparks */}
                        {[[130, 50], [170, 55], [140, 110], [160, 130], [130, 185], [170, 180]].map(([x, y], i) => <circle key={i} cx={x} cy={y} r="1.5" fill={ac} opacity=".3" style={{ animation: `neuralSpark ${1.5 + i * .3}s ease infinite ${i * .2}s` }} />)}
                      </svg>
                    </div>
              ...
              {dashSections.activity && <>
                ...
                {records.topProto && <div style={{ background: cd, borderRadius: 16, padding: "12px", marginBottom: 14, border: `1px solid ${bd}` }}>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 5 }}>
                    {[{ v: records.bestStreak, l: "Mejor racha", s: "días" }, { v: records.maxC + "%", l: "Max coherencia", s: "" }, { v: records.topProto.c + "x", l: records.topProto.n, s: "" }, { v: records.earliest !== null ? records.earliest + ":00" : "—", l: "Más temprana", s: "" }].map((r, i) => <div key={i} style={{ padding: "8px", background: isDark ? "#1A1E28" : "#F8FAFC", borderRadius: 10 }}>
        ...
        <div style={{ background: `linear-gradient(135deg,${isDark ? "#141820" : "#F0F4FF"},${isDark ? "#1A1E28" : "#F8FAFC"})`, borderRadius: 16, padding: "16px 14px", marginBottom: 10, border: `1px solid ${isDark ? "#2A2E3A" : "#D4DDEF"}` }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 6, marginBottom: 10 }}>
            {[{ n: "Dashboard\nde equipo", ic: "chart" }, { n: "Métricas\nde bienestar", ic: "up" }, { n: "Challenges\ngrupales", ic: "trophy" }].map((f, i) => <div key={i} style={{ textAlign: "center", padding: "8px 4px", background: cd, borderRadius: 10, border: `1px solid ${bd}` }}>
    ...
    </div><div style={{ position: "fixed", bottom: 58, left: "50%", transform: "translateX(-50%)", width: "100%", maxWidth: 430, padding: "5px 20px", background: `${bg}EB`, backdropFilter: "blur(14px)", display: "flex", justifyContent: "center", gap: 12, zIndex: 50, borderTop: `1px solid ${bd}` }}>
      ...
      </div><div style={{ position: "fixed", bottom: 0, left: "50%", transform: "translateX(-50%)", width: "100%", maxWidth: 430, background: `${isDark ? "rgba(11,14,20,.94)" : "rgba(255,255,255,.94)"}`, backdropFilter: "blur(18px)", borderTop: `1px solid ${bd}`, padding: "3px 10px 10px", display: "flex", justifyContent: "center", zIndex: 60 }}>
        {[{ id: "ignicion", lb: "Core", ic: "bolt" }, { id: "dashboard", lb: "Estado", ic: "chart" }, { id: "perfil", lb: "Yo", ic: "user" }].map(t => {
          const a = tab === t.id; return (<button key={t.id} onClick={() => switchTab(t.id)} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 1, padding: "6px 0 1px", border: "none", cursor: "pointer", background: a ? (isDark ? "#1A1E28" : "#E8ECF4") : "transparent", borderRadius: 11, margin: "0 2px" }}>
