/* ═══════════════════════════════════════════════════════════════
   E.164 country registry — ISO 3166-1 alpha-2 + dial code +
   localized display name. Used by the phone sign-in combobox.
   Kept inline (no fetch) so the picker renders instantly.
   ═══════════════════════════════════════════════════════════════ */

// Sorted alphabetically by english name for predictable search.
// `name` is English; we localize via display names on render when
// `navigator` exposes `Intl.DisplayNames`.
export const COUNTRIES = [
  ...
  { iso: "MK", dial: "389",  name: "Macedonia" },
...
// Pull localized country names via Intl when available; falls back to English.
// Only called client-side inside the picker.
export function localizedName(iso, locale) {
  try {
    const dn = new Intl.DisplayNames([locale || "en"], { type: "region" });
    return dn.of(iso) || BY_ISO[iso]?.name || iso;
  } catch {
    return BY_ISO[iso]?.name || iso;
  }
}

// Best-effort autodetect: browser's navigator.language region ("es-MX" → "MX").
export function detectIso() {
  if (typeof navigator === "undefined") return "MX";
  const lang = navigator.language || "es-MX";
  const match = lang.match(/-([A-Z]{2})/i);
  const iso = match ? match[1].toUpperCase() : null;
  return iso && BY_ISO[iso] ? iso : "MX";
}
